...
export class Migration20230812134215 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "group" ("id" uuid not null, "name" varchar(255) not null, "search_index" tsvector null, "created_at" timestamptz(0) null, "updated_at" timestamptz(0) null, constraint "group_pkey" primary key ("id"));');
    ...
    this.addSql('create table "attribute_option" ("id" uuid not null, "value" varchar(255) not null, "display_name" varchar(255) not null, "attribute_id" uuid not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) null, constraint "attribute_option_pkey" primary key ("id"));');
    this.addSql('alter table "attribute_option" add constraint "attribute_option_display_name_unique" unique ("display_name");');

    this.addSql('create table "manufacturer" ("id" uuid not null, "name" varchar(255) not null, "search_index" tsvector not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) null, constraint "manufacturer_pkey" primary key ("id"));');
    this.addSql('create index "manufacturer_search_index_index" on "public"."manufacturer" using gin("search_index");');

    this.addSql('create table "part" ("id" varchar(255) not null, "name" varchar(255) not null, "status" text check ("status" in (\'in-stock\', \'out-of-stock\')) not null default \'in-stock\', "search_index" tsvector not null, "properties" jsonb null, "manufacturer_id" uuid null, "category_id" uuid null, "base_price" int not null default 0, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) null, "publish_status" varchar(255) null default \'draft\', constraint "part_pkey" primary key ("id"));');
    this.addSql('create index "part_search_index_index" on "public"."part" using gin("search_index");');

    this.addSql('create table "option_config" ("id" uuid not null, "price" int not null default 0, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) null, "option_id" uuid null, "part_id" varchar(255) not null, constraint "option_config_pkey" primary key ("id"));');
    ...
    this.addSql('alter table "part" add constraint "part_manufacturer_id_foreign" foreign key ("manufacturer_id") references "manufacturer" ("id") on update cascade on delete set null;');
